...
<%-- view.jsp 
	id 파라미터 값을 board 테이블의  board_id 컬럼으로 조회하여
	board_id와 id 파라미터가 일치하는 게시글 데이터들을 가지고와서 화면에 출력한다.
	--%>
...
Board data = null;		// select sql문의 결과는 Board 클래스 객체 하나에 저장되는 데이터이기 때문에
				// Board 클래스 객체를 저장할 수 있는 변수만 있으면 된다.
try{
...
// 해당 게시글의 이미지 가져오기
	...
	try{
		...
		e.printStackTrace();
		System.out.println("게시글 이미지 조회실패");
